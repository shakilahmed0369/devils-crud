...
```
Once the package is installed, you should register the `Nox\DevilsCrud\CrudGeneratorServiceProvider` service provider. Normally, Laravel 5.5+ will register the service provider automatically.

After that, publish its assets using the `vendor:publish` Artisan command:
```
php artisan vendor:publish --provider="Nox\DevilsCrud\CrudGeneratorServiceProvider"
```
...
public function register()
{
    if ($this->app->environment() == 'local') {
        $this->app->register('Nox\DevilsCrud\CrudGeneratorServiceProvider');
    }
}
```
